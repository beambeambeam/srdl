...
async function applyWithTransition(origin: { x: number; y: number }, apply: () => void) {
...
function ThemeSwitch({ iconSize = 16, className }: ThemeSwitchProps) {
  ...
  const toggle = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    void applyWithTransition(originRef.current, () => setTheme(next));
  };
      ...
      <AnimatePresence mode="wait" initial={false}>
        {isDark ? (
          <motion.span
            ...
            transition={{ bounce: 0.3, duration: 0.28, type: "spring" }}
