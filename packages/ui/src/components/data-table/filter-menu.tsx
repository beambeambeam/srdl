/* eslint-disable complexity, no-nested-ternary, no-shadow, no-use-before-define */
...
import { Calendar } from "@srdl/ui/components/calendar";
import {
  Command,
  CommandEmpty,
  CommandGroup,
  ...
  CommandItem,
  CommandList,
} from "@srdl/ui/components/command";
import { Input } from "@srdl/ui/components/input";
import { Popover, PopoverContent, PopoverTrigger } from "@srdl/ui/components/popover";
import {
  Select,
  SelectContent,
  SelectItem,
...
import { DataTableRangeFilter } from "@srdl/ui/components/data-table/range-filter";
import { useDebouncedCallback } from "@srdl/ui/hooks/use-debounced-callback";
import { getDefaultFilterOperator, getFilterOperators } from "@srdl/ui/lib/data-table";
import { formatDate } from "@srdl/ui/lib/format";
import { generateId } from "@srdl/ui/lib/id";
import { getFiltersStateParser } from "@srdl/ui/lib/parsers";
import { cn } from "@srdl/ui/lib/utils";
import type { ExtendedColumnFilter, FilterOperator } from "@srdl/ui/types/data-table";
...
export function DataTableFilterMenu<TData>({
  ...
  const onInputKeyDown = React.useCallback(
    (event: React.KeyboardEvent<HTMLInputElement>) => {
      if (REMOVE_FILTER_SHORTCUTS.has(event.key.toLowerCase()) && !inputValue && selectedColumn) {
  ...
  const onTriggerKeyDown = React.useCallback(
    (event: React.KeyboardEvent<HTMLButtonElement>) => {
      if (REMOVE_FILTER_SHORTCUTS.has(event.key.toLowerCase()) && filters.length > 0) {
        event.preventDefault();
